...
import {Orientation,PresetId} from "./lib/types";
let els:ElementsMap;
...
function init(){
  ...
  const change=(kind:"preset"|"orientation")=>(e:Event)=>{busy();const t=e.target as HTMLInputElement;if(t.checked)store.update({[kind]:t.value} as Partial<Record<typeof kind,PresetId|Orientation>})};
